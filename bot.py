...
async def init_stream_pool():
    ...
    for i in range(POOL_SIZE):
        c = Client(
            ...
            api_hash       = Config.API_HASH,
            bot_token      = Config.BOT_TOKEN,
            session_string = Config.STRING_SESSION,  # same session = same DC auth cache
            in_memory      = True,
            workers        = 0,   # ← KEY FIX: do NOT process updates
        )                         #   without this, stream clients steal all
                                  #   messages from the main bot
        await c.start()
