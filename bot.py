...
async def create_dc_client(dc_id: int) -> Client:
    """
    Create and return a Pyrogram client authorized for dc_id.

    Steps:
      1. Export authorization from main bot session for target DC
      2. Create a new bare Client connected to that DC
      3. Import the exported authorization into the new client
      4. Client is now fully authorized for dc_id without ExportAuthorization
         ever being needed again during streaming
    """
    # Export auth from main bot → target DC
    exported = await bot.invoke(
        raw_fn.auth.ExportAuthorization(dc_id=dc_id)
    )

    # Create a new client but do NOT pass bot_token — it will use imported auth
    client = Client(
        f"dc{dc_id}",
        api_id   = Config.API_ID,
        api_hash = Config.API_HASH,
    ...
    # Connect at the raw level so we can import auth before full start
    await client.connect()

    # Tell this client to use the exported DC
    await client.storage.dc_id(dc_id)
    await client.storage.is_bot(False)

    # Import the authorization — client is now authorized on dc_id
    await client.invoke(
