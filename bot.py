...
async def create_dc_client(dc_id: int) -> Client:
    ...
    CORRECT 3-STEP METHOD FOR BOTS:
    ─────────────────────────────────────────────────────────────────────
    Step 1: Export authorization FROM the main bot session (DC5) FOR
            the target DC.  This gives us auth bytes valid on dc_id.

    Step 2: Create a new bare client, connect it directly to dc_id,
            mark storage as is_bot=True  ← this was the missing fix
            (previous attempt used is_bot=False → AUTH_BYTES_INVALID,
             ImportBotAuthorization → USER_MIGRATE because Telegram
             redirects fresh bot login back to the home DC)

    Step 3: ImportAuthorization with the exported bytes.
            Now this client is fully authorized ON dc_id as a bot.
            stream_media on this client = direct DC connection, no
            ExportAuthorization ever needed during downloads.
    """
    ...
    # Step 2 — create client, connect to target DC, mark as bot ✅
    client = Client(
        f"dc{dc_id}",
        api_id    = Config.API_ID,
        api_hash  = Config.API_HASH,
    ...
    await client.storage.is_bot(True)   # ← CRITICAL: must be True for bots

    # Step 3 — import the exported auth bytes into this DC client
