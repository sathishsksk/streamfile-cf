...
STRING SESSION Edition — Permanent FloodWait Fix

ROOT CAUSE OF FloodWait:
  in_memory=True → session wiped on every restart
  Every restart → Pyrogram calls auth.ExportAuthorization for each DC
  Telegram rate limits ExportAuthorization → FloodWait 2850s

REAL FIX — STRING_SESSION:
  1. Generate session string ONCE using gen_session.py
  2. Store as STRING_SESSION env var in Koyeb
  3. Pyrogram loads the full session including cached DC auth
  4. DC auth is already in the session → ExportAuthorization NEVER called
  5. FloodWait impossible — even across restarts
...
All previous fixes also included:
  ...
  ✅ FloodWait caught        graceful error if ever triggered
"""
...
# ── Bot client — uses STRING_SESSION for persistent DC auth cache ─────────────
bot = Client(
    "session",
    ...
    in_memory   = True,   # fallback if no STRING_SESSION provided
)
...
async def stream_handler(request: web.Request):
    ...
    chunk_index fix: offset = chunk NUMBER not bytes.
    FloodWait: should never happen with STRING_SESSION.
               Caught gracefully if it ever does.
    """
    ...
    # Pre-fetch first chunk so we can still return error before headers sent
    media_iter  = bot.stream_media(file_id, offset=chunk_index).__aiter__()
    first_chunk = None
    try:
        raw         = await media_iter.__anext__()
        first_chunk = raw[skip_bytes:]
    except FloodWait as e:
        log.warning(f"[FloodWait] {token} — {e.value}s — update STRING_SESSION env var!")
        return web.Response(
            status  = 503,
            text    = f"Telegram rate limit. Retry in {e.value}s.",
...
<p>Pyrogram MTProto · String Session · 4 GB Support</p>
<div class="badges">
...
async def main():
    mode = "String Session" if Config.STRING_SESSION else "in_memory (⚠️ set STRING_SESSION!)"
    ...
    # Export auth for all DCs once at startup so they are cached in session
    if Config.STRING_SESSION:
        log.info("🔄 Pre-warming DC auth cache…")
        ...
        for dc_id in [1, 2, 3, 4, 5]:
            if dc_id == main_dc:
                continue
            try:
                await bot.invoke(raw_fn.auth.ExportAuthorization(dc_id=dc_id))
                log.info(f"✅ DC{dc_id} auth cached")
            except FloodWait as e:
                log.warning(f"⏳ DC{dc_id} FloodWait {e.value}s — skipping pre-warm")
            except Exception as e:
                log.warning(f"⚠️ DC{dc_id} pre-warm skipped: {e}")
        log.info("✅ DC auth pre-warm complete — FloodWait will not occur")
