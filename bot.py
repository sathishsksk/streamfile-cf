...
✅ FloodWait handled       503 returned gracefully before headers sent
"""
...
async def stream_handler(request: web.Request):
    ...
    Two key fixes:
    1. chunk_index fix  — offset = chunk NUMBER not bytes (fixes OFFSET_INVALID)
    2. Prefetch fix     — fetch first chunk BEFORE response.prepare()
                          so we can return 503 if FloodWait hits,
    ...
    # Fix 1 — chunk index (not bytes)
    chunk_index = byte_start // CHUNK_SIZE
    skip_bytes  = byte_start % CHUNK_SIZE

    # Fix 2 — prefetch first chunk BEFORE headers are sent
    # so FloodWait can still return a proper 503 response
    ...
    # Safe to send headers now — first chunk already fetched
    headers = {
        "Content-Type"               : mime,
    ...
    try:
        ...
        # Client disconnected before headers were sent — not a bot error, ignore
        return response
    ...
    try:
        ...
        log.warning(f"[FloodWait mid-stream] {token} — {e.value}s — closing")
    except FileIdInvalid:
