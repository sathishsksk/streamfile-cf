...
async def create_dc_client(dc_id: int) -> Client:
    ...
    WHY THE PREVIOUS VERSION FAILED:
      ExportAuthorization + ImportAuthorization = USER ACCOUNT method only.
      For BOT accounts, Telegram returns AUTH_BYTES_INVALID on import.

    CORRECT METHOD FOR BOTS:
      Use auth.ImportBotAuthorization directly on each DC.
      This re-authorizes the bot token on that DC without any export step.
      Works on ALL DCs regardless of which DC the main session is on.
    """
    client = Client(
        f"dc{dc_id}",
        api_id   = Config.API_ID,
    ...
    # Connect the client to the correct DC
    await client.connect()
    await client.storage.dc_id(dc_id)
    await client.storage.is_bot(True)

    # ✅ BOT-CORRECT method: ImportBotAuthorization directly on this DC
    # No export from main session needed — bot token works on all DCs
    result = await client.invoke(
        raw_fn.auth.ImportBotAuthorization(
            flags          = 0,
            api_id         = Config.API_ID,
            api_hash       = Config.API_HASH,
    ...
    # Store the authorized user id so the session is considered valid
    await client.storage.user_id(result.user.id)
    await client.storage.date(0)

    log.info(f"✅ DC{dc_id} client ready (bot @{result.user.username})")
    return client
