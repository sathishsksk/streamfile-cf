...
All fixes applied:
  ✅ in_memory=True (no session file needed on Koyeb)
  ✅ enums.ParseMode.HTML (Pyrogram 2.x compatible)
  ✅ bot.run() entry point (proper event loop)
  ✅ web server as background task (no blocking)
  ✅ idle() from pyrogram (proper update loop)
"""
...
def get_media_info(msg: Message):
    """Extracts file info from any message type. Supports up to 4 GB via MTProto."""
    media = (
    ...
    if not name:
        types = {
            ...
            "photo": "jpg", "sticker": "webp", "animation": "gif",
            "video_note": "mp4",
        }
        for t, ext in types.items():
            if getattr(msg, t, None):
                name = f"{t}_{msg.id}.{ext}"
                break
...
async def cmd_help(_, msg: Message):
    await msg.reply_text(
        ...
        "Just send any <b>file, video, audio, photo</b> to get a download link!",
        parse_mode=enums.ParseMode.HTML,
    )
...
async def handle_file(client, msg: Message):
    ...
    # Copy to BIN_CHANNEL via MTProto (supports up to 4 GB)
    try:
        ...
        await proc.edit_text(
            ...
            "Make sure the bot is added to <b>BIN_CHANNEL as Admin</b>.",
            parse_mode=enums.ParseMode.HTML,
        )
        return

    # Use fresh file_id from BIN_CHANNEL copy
    bin_info = get_media_info(fwd)
    if bin_info:
        info["file_id"] = bin_info["file_id"]

    token = await save_file(info, fwd.id)
    dl_url, page_url = build_links(token)
    ...
    await proc.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup([[
            ...
            InlineKeyboardButton("🌐 Web Page", url=page_url),
        ]]),
    )
...
async def handle_text(_, msg: Message):
    if not Config.MY_PASS:
        return
    if not await is_pending(msg.from_user.id):
        return
...
async def stream_handler(request: web.Request):
    """GET /stream/{token} — streams file bytes via Pyrogram. Supports Range requests."""
...
async def info_handler(request: web.Request):
    """GET /info/{token} — returns file metadata as JSON for Cloudflare Worker."""
...
async def health_handler(_):
    """GET /health — for UptimeRobot + Koyeb health check."""
...
def build_web_app():
    ...
    app.router.add_get("/info/{token}",   info_handler)
    return app
...
async def main():
    ...
    # Start web server as background task (non-blocking)
    ...
    # Pyrogram idle — properly receives all updates
    await idle()
...
if __name__ == "__main__":
    # bot.run() is Pyrogram's official entry point
    # It manages the asyncio event loop correctly
    bot.run(main())
