...
✅ chunk_index fix         offset=chunk NUMBER not bytes → fixes OFFSET_INVALID
"""
...
async def handle_file(client, msg: Message):
    ...
    if not info: return
    proc = await msg.reply_text("⏳ <b>Processing your file…</b>", parse_mode=enums.ParseMode.HTML)
    try:
        fwd = await client.copy_message(Config.BIN_CHANNEL, msg.chat.id, msg.id)
    except Exception as e:
        log.error(f"copy_message failed: {e}")
        await proc.edit_text("❌ <b>Failed.</b> Is bot Admin in BIN_CHANNEL?", parse_mode=enums.ParseMode.HTML)
        return
...
async def handle_text(_, msg: Message):
    ...
    if msg.text == Config.MY_PASS:
        ...
        await msg.reply_text("✅ <b>Correct! Now send your file.</b>", parse_mode=enums.ParseMode.HTML)
    else:
        await msg.reply_text("❌ <b>Wrong password.</b> Try again.", parse_mode=enums.ParseMode.HTML)
...
async def handle_channel(client, msg: Message):
    ...
    try:
        ...
        token = await save_file(info, fwd.id)
        dl, page = build_links(token)
        await client.edit_message_reply_markup(msg.chat.id, msg.id,
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("⬇️ Download", url=dl),
                InlineKeyboardButton("🌐 Web Page", url=page),
            ]]))
...
async def stream_handler(request: web.Request):
    ...
    THE CRITICAL FIX FOR OFFSET_INVALID:
    ─────────────────────────────────────
    Pyrogram stream_media `offset` = CHUNK INDEX (0, 1, 2 ...)
                                     NOT byte offset!
    Each chunk = exactly 1 MB = 1,048,576 bytes

    WRONG:  stream_media(file_id, offset=5000000)   ← raw bytes → CRASH
    RIGHT:  stream_media(file_id, offset=4)          ← chunk #4 = bytes 4MB–5MB

    Formula:
      chunk_index = byte_start // 1_048_576
      skip_bytes  = byte_start %  1_048_576
    """
    ...
    # ✅ THE FIX: convert byte offset → chunk index
    chunk_index = byte_start // CHUNK_SIZE   # which chunk number to start at
    skip_bytes  = byte_start % CHUNK_SIZE    # bytes to skip inside that chunk
...
async def info_handler(request: web.Request):
    ...
    if not info: return web.json_response({"error":"not found"}, status=404)
...
async def health_handler(_): return web.Response(text="OK")
...
.b{text-align:center}h1{color:#0088cc;margin-bottom:12px}p{color:#888}
.ok{color:#22c55e;margin-top:16px}</style></head>
<body><div class="b"><h1>📁 File To Link Bot</h1>
<p>Pyrogram MTProto · 4 GB Support</p>
<p style="font-size:.8rem;color:#555">Koyeb + Cloudflare Worker</p>
<div class="ok">✅ Running on Koyeb</div></div></body></html>""")
